...
if ( ! class_exists( 'TR_Admin_Page_Class_V4') ) :
 ...
 */
  ...
  class TR_Admin_Page_Class_V4 {
    ...
    public function AddMenuTopPage() {
      ...
      $id = add_menu_page($this->args['menu_title'], $this->args['menu_title'], $this->args['capability'], $this->args['id'], array($this, 'Menu_Callback'), $this->args['icon_url'], $this->args['position']);
      $page = add_submenu_page($id, $this->args['menu_title'], $this->args['menu_title'], $this->args['capability'], $this->args['id'], array($this, 'Menu_Callback'));
      if ($page){
         $this->_Slug = $page;
        ...
        jQuery(document).ready(function() {  
          function show_tab(li){
            if (!jQuery(li).hasClass("active_tab")){
              ...
              jQuery(".panel_menu li").removeClass("active_parent_tab");
          ...
          if (last_tab == null){
            ...
            var la = jQuery(\'[href="\' + last_tab + \'"]\');
            if(la.hasClass(\'nav_tab_link\'))
                show_tab(la.parent());
            else{jQuery(".panel_menu li:first").addClass("active_tab");
            var tab  = jQuery(".panel_menu li:first a").attr("href");
            jQuery(tab).show();}
          }
      
          //bind click on menu action to show the right tab.
          jQuery(".panel_menu li a").bind("click", function(event){
            event.preventDefault()
            show_tab(jQuery(this).parent("li"));
            ...
            <ul>
                ...
                foreach($tabs as $tabslug => $tabtext):
                    ...
                    <li class="<?php if($active_tab)echo 'active_tab'?>">
                        <a href="?page=<?php echo $_GET['page']?>&<?php echo $slug?>=<?php echo $tabslug?>">
                        <?php echo $tabtext?></a>
                    </li>
                <?php endforeach;?>
            </ul>
    ...
    /**
      ...
      public function DisplayPage() {
        ...
        echo '<form method="post" action="" id="admin_options_form" enctype="multipart/form-data">
          ...
          $buttons = '<input type="button" style="margin-left: 20px;" value="Restore Default" class="btn-warning btn restore_button">';
  ...
  public function load_scripts_styles() {
    ...
    // @TODO only load styles and js when needed
      ...
      wp_enqueue_style( 'Admin_Page_Class', $plugin_path . '/css/admin.css',array(),'1.1.2' );
      wp_enqueue_style('iphone_checkbox',$plugin_path. '/js/iphone-style-checkboxes/style.css');
      
      // Enqueue admin page Scripts
      wp_enqueue_script( 'Admin_Page_Class', $plugin_path . '/js/Admin_Page_Class.js', array( 'jquery' ), '1.1.4', true );
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_color( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      echo "<input class='at-color' type='color' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='8' style='background-color:{$meta}' />";
  ...
  function Handle_plupload_action(){
    ...
    $status = $this->move_upload_folder($status,$imgid);
    
    echo $status['url'];
    exit;
  }
  
  function move_upload_folder($status,$imgid)
  {
    //find field 
    $has_field = false;
    foreach ( $this->_fields as $field ) {
